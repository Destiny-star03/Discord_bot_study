...
from crawler.dept_notice_detail import fetch_notice_detail
...
def _load_last_id() -> str | None:
    ...
    try:
        with open(STATE_FILE, "r", encoding="utf-8") as f:
            data = json.load(f)
        return data.get("last_dept_notice_id")
    except Exception:
        return None


def _save_last_id(last_id: str) -> None:
    data = {"last_dept_notice_id": last_id}
...
class DeptNoticeWatcher:
    ...
    async def loop(self):
        ...
        channel = self.bot.get_channel(DEPT_NOTICE_CHANNEL_ID)
        if channel is None:
            try:
                channel = await self.bot.fetch_channel(DEPT_NOTICE_CHANNEL_ID)
            except Exception:
                return
        ...
        try:
            notices = await asyncio.to_thread(fetch_dept_notices, DEPT_NOTICE_URL, 3)
        except Exception:
            return
